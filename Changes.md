...
- **`interpolate_color(override_color=None)` parameter** — when set, replaces `values.color` for the preview; matches what `mat.override(color=override_color)` would render at runtime, without materializing a new `PbrProperties`. Multiplied against the color-texture average (when present) in linear space, then sRGB-encoded for the returned tuple. Useful for CAD-viewer preview color computation when a tint will be applied to a textured material.
